...
import libs.emailprotectionslib.dmarc as dmarclib
import libs.emailprotectionslib.spf as spflib
...
def check_spf_all_string(spf_record):
    ...
    if spf_record.all_string is not None:
        if spf_record.all_string == "~all" or spf_record.all_string == "-all":
            record_more = "SPF record contains an All item: " + spf_record.all_string
            output_indifferent(record_more)
        else:
            record_more = "SPF record All item is too weak: " + spf_record.all_string
            output_good(record_more)
            strong_spf_all_string = False
    else:
        record_more = "SPF record has no All string"
        output_good(record_more)
        strong_spf_all_string = False
    
    # save into the db obj
    data['spf_record_more'] = record_more
...
def is_spf_record_strong(domain):
    ...
    if spf_record is not None and spf_record.record is not None:
        ...
        # save into the db obj
        data['spf_record'] = str(spf_record.record)
...
def get_dmarc_record(domain):
    ...
    if dmarc is not None and dmarc.record is not None:
        ...
        # save into the db obj
        data['spf_dmarc'] = str(dmarc.record)
    return dmarc
...
# trigger func for spoofcheck
def _run_spoofcheck(domain):
    print('[***] spoofcheck [***]')
    spoofable = False
    spoofable_str = None
    try:
        ...
        if spoofable:
            spoofable_str = "Spoofing possible for " + domain + "!"
            output_good(spoofable_str)
        else:
            spoofable_str = "Spoofing not possible for " + domain
            output_bad(spoofable_str)

        data['spf_spoofing_possible'] = spoofable_str
    except IndexError:
